...
주소를 넣지 않고 빌드해도 앱은 돕니다. 아래 "문서 읽기" 쪽 규칙이 대신 읽고, 결과에 "AI 없이 문서에서 직접 읽었습니다" 라고 뜹니다.

`functions/index.js` 는 `claude-opus-5` 를 씁니다. 부르는 곳은 `web/src/App.jsx` 의 `askClaude` 입니다.

## 문서 읽기 (API 키 없이)

키가 없어도, 프록시가 죽어도 문서에서 글자를 긁어 할 일을 뽑습니다. 전부 브라우저 안에서 돌고 아무 데도 파일을 보내지 않습니다.

`web/src/extract.js` — 글자 꺼내기

| 형식 | 방법 |
| --- | --- |
| PDF | 객체·스트림을 직접 풀고(Flate, PNG 예측기, 객체 모음) 글꼴의 ToUnicode 표를 따라 글자를 복원 |
| HWPX | zip 을 직접 열어 `Contents/section*.xml` 의 글자만 추림 |
| HWP 5.0 | OLE 복합 문서를 훑어 `BodyText/Section*` 을 풀고 문단 글자 기록(태그 67)만 뽑음 |
| 엑셀·워드 | `xlsx`, `mammoth` (파일을 넣을 때만 내려받음) |
| 이미지·스캔 PDF | 글자가 그림 속이라 여기서는 못 읽습니다. 이때만 AI 프록시로 넘어갑니다 |

`web/src/scan.js` — 글자에서 뽑기

- 날짜: `2026. 9. 11.(금)`, `9/11`, `9월 11일` 같은 표기와 "담주 화", "이달 말", "3일 뒤" 같은 상대 표현. 연도가 없으면 가장 가까운 앞날로, 요일이 적혀 있으면 그 요일에 맞는 해로 고릅니다.
- 마감 고르기: 날짜 앞뒤의 "제출기한", "까지", "회신" 같은 말에 점수를 매겨 행사일과 갈라냅니다. 기간(`~`)이면 끝날을 씁니다. 토·일 마감은 앞선 금요일로 당깁니다.
- 나머지 칸: 제목은 그 항목의 머리줄에서, 담당·제출방법·부서는 문서 안의 해당 줄에서 가져오고, 마감 근거가 된 원문을 그대로 남깁니다.
- 학사일정과 시간표도 같은 방식으로 뽑습니다(`scanEvents`, `scanTimetable`).

AI 를 켜 두면 AI 를 먼저 쓰고, 부르지 못하거나 빈손으로 오면 이 규칙 결과로 갈아탑니다.

점검:

```bash
node web/read-check.mjs
```
